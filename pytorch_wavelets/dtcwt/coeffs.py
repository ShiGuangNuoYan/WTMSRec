...
def _load_from_file(basename, varnames):
    ...
    try:
        ...
        with resource_stream('pytorch_wavelets_1.dtcwt.data', basename + '.npz') as f:
            mat = dict(load(f))
...
def biort(name):
    """ Deprecated. Use :py::func:`pytorch_wavelets_1.dtcwt.coeffs.level1`
    Instead
    """
...
def level1(name, compact=False):
    ...
    :raises ValueError: if name doesn't specify
        :py:func:`pytorch_wavelets_1.dtcwt.coeffs.qshift` wavelet.
...
def qshift(name):
    ...
    :raises ValueError: if name doesn't specify a
        :py:func:`pytorch_wavelets_1.dtcwt.coeffs.biort` wavelet.
